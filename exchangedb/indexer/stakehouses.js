...
/* Import handlers. */
import handleStakehouse from '../handlers/stakehouse.js'
...
export default async (_curHeight = 0) => {
    console.info('\n  Checking STAKEHOUSES database sync...\n')
    ...
    if (_curHeight > systemIdx?.last) {
        console.info('\n  Starting STAKEHOUSES database sync...\n')
        ...
        for (let i = systemIdx.last + 1; i <= _curHeight; i++) {
            ...
            if (block?.txidem) {
                for (let j = 0; j < block.txidem.length; j++) {
                    ...
                    /* Handle Stakehouse transactions. */
                    await handleStakehouse(tx)
                }
            }
