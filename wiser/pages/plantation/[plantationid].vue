...
const plantationTicker = computed(() => {
    return plantation.value?.ticker || ''
})

const plantationIcon = computed(() => {
    if (!plantation.value) {
        return null
    }
...
const plantationUrl = computed(() => {
    ...
    return plantation.value?.document_url || null
})

const homepageUrl = computed(() => {
    /* Validate plantation URL. */
    if (!plantationUrl.value) {
        return null
    }

    /* Decode URL. */
    const url = new URL(plantationUrl.value)
    console.log('URL', url)
...
onMounted(async () => {
    ...
    plantation.value = await $fetch('/api/plantation/' + plantationid.value)
        .catch(err => console.error(err))
    console.log('PLANTATION DETAILS', plantation.value)
                    ...
                    <div class="mt-6">
                        ...
                        <section>
                            ...
                            <h3 class="text-5xl font-bold text-indigo-600">
                                ...
                                <span class="ml-2 text-base">per ${{plantationTicker}}</span>
                            </h3>
                        </section>
                            ...
                            <div>
                                ...
                                <div class="prose prose-sm pb-6" id="disclosure-1">
                                    ...
                                    <div class="mt-5 grid grid-cols-1 sm:grid-cols-3 gap-1 sm:gap-4 bg-sky-100">
                                        ...
                                        <NuxtLink :to="plantationUrl" target="_blank" class="text-sm text-sky-900 font-medium text-center sm:text-left group px-2 py-1 col-span-1 sm:col-span-2 bg-sky-200 hover:bg-sky-300">
                                            {{plantationUrl}}
                                ...
                                <div class="prose prose-sm pb-6" id="disclosure-1">
                                    ...
                                    <div v-if="plantationDoc?.[0].category" class="mt-5 grid grid-cols-1 sm:grid-cols-3 gap-1 sm:gap-4 bg-sky-100">
                                        <div class="px-2 py-1 text-sm text-sky-900 font-medium text-center sm:text-right bg-sky-200">
                                            Category
                                        </div>
                                        ...
                                        <div class="text-sm text-sky-900 font-medium text-center sm:text-left group px-2 py-1 col-span-1 sm:col-span-2 bg-sky-200 hover:bg-sky-300">
                                            {{plantationDoc[0].category}}
                                        </div>
                                    </div>

                                    <div v-if="plantationDoc?.[0].legal" class="mt-5 grid grid-cols-1 sm:grid-cols-3 gap-1 sm:gap-4 bg-sky-100">
                                        <div class="px-2 py-1 text-sm text-sky-900 font-medium text-center sm:text-right bg-sky-200">
                                            Legal
                                        </div>

                                        <div class="text-sm text-sky-900 font-medium text-center sm:text-left group px-2 py-1 col-span-1 sm:col-span-2 bg-sky-200 hover:bg-sky-300">
                                            {{plantationDoc[0].legal}}
                                        </div>
                                    </div>
                                    ...
                                    <div class="mt-5 grid grid-cols-1 sm:grid-cols-3 gap-1 sm:gap-4 bg-sky-100">
                                        ...
                                        <div class="text-sm text-sky-900 font-medium text-center sm:text-left group px-2 py-1 col-span-1 sm:col-span-2 bg-sky-200 hover:bg-sky-300">
                                            <p class="px-3 py-2 grid-cols-1 sm:col-span-2 text-xs truncate">{{JSON.stringify(plantationDoc?.[0].contact, null, 2)}}</p>
                                            <!-- <pre class="hidden lg:block px-3 py-2 grid-cols-1 sm:col-span-2 text-sm">{{JSON.stringify(plantationDoc?.[0].contact, null, 2)}}</pre> -->
                                        </div>
                                    </div>
